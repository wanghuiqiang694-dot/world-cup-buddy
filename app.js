...
async function loadCloudData() {
  try {
    ...
    if (wcResp.ok) {
      ...
      // 比分优先用云端（GitHub Actions用searchevents逐场查询生成，比eventsday完整）
      if (wcData.matchResults) {
        let updated = 0;
        for (const [id, result] of Object.entries(wcData.matchResults)) {
          if (!MATCH_RESULTS[id] || MATCH_RESULTS[id].score !== result.score) {
            MATCH_RESULTS[id] = result;
            updated++;
          }
        }
        if (updated > 0) console.log('[云端] 世界杯比分更新：' + updated + ' 条');
      }
      // 如果云端赛程比硬编码多，才覆盖
      if (wcData.matches && wcData.matches.length > MATCHES.length) {
        MATCHES.length = 0;
    ...
    if (mcResp.ok) {
      ...
      if (mcData.standings) { MENGCHAO_STANDINGS.length = 0; mcData.standings.forEach(function(s) { MENGCHAO_STANDINGS.push(s); }); }
      if (mcData.matches) { MENGCHAO_MATCHES.length = 0; mcData.matches.forEach(function(m) { MENGCHAO_MATCHES.push(m); }); }
      if (mcData.scorers) { MENGCHAO_SCORERS.length = 0; mcData.scorers.forEach(function(s) { MENGCHAO_SCORERS.push(s); }); }
    ...
    try {
      ...
      localStorage.setItem('wc_cloud_mengchao', JSON.stringify({ teams: MENGCHAO_TEAMS, standings: MENGCHAO_STANDINGS, matches: MENGCHAO_MATCHES, scorers: MENGCHAO_SCORERS }));
      if (typeof LEAGUE_DATA !== 'undefined') {
        leagueKeys.forEach(function(lk) { try { localStorage.setItem('wc_cloud_' + lk, JSON.stringify(LEAGUE_DATA[lk])); } catch (e) {} });
      }
    } catch (e) {}
    showRefreshIndicator('数据已从云端同步');
  } catch (err) {
    console.warn('[云端] 数据加载失败，使用缓存数据：', err.message);
    try {
...
async function fetchLiveScores() {
  try {
    ...
    const tomorrowStr = tomorrow.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0') + '-' + String(tomorrow.getDate()).padStart(2, '0');
    
    const recentMatches = MATCHES.filter(m => m.date === todayStr || m.date === tomorrowStr);
    if (recentMatches.length === 0) { if (!liveRefreshTimer) liveRefreshTimer = setInterval(fetchLiveScores, LIVE_REFRESH_INTERVAL); return; }
    ...
    if (updated > 0) showRefreshIndicator('实时更新 ' + updated + ' 条');
  } catch (err) {
